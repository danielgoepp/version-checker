...
## Terminal UI
`check_versions.py --tui` launches a full-screen Textual app (`src/tui/app.py`) that wraps `VersionManager` — it calls the exact same `check_all_applications()` / `upgrade_rows()` methods the CLI path uses, no parallel logic. It starts in an "Updates" view (only apps with `status: Update Available`), which is a read filter over `vm.notes`, not a separate data source.
