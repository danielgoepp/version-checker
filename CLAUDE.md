...
- **API Caching**: LRU caching on GitHub and Docker Hub API calls dedupes lookups for multi-instance apps. The caches are cleared at the start of every `check_all_applications()` run, so they scope to a run, not the process — a long-lived TUI session gets fresh latest-version results on each check-all
- **Checker Dispatch**: `CURRENT_CHECKERS` in `version_manager.py` is a name-keyed registry mapping each application to its current-version checker (small adapter callables over the checker modules). Adding an app's checker is one dict entry; `ssh`/`ssh_apt` rows are dispatched by method instead of name since their names describe host classes (`rpi`, `ubuntu`, ...)
- **Security**: No shell=True in subprocess calls - all commands use list-based construction to prevent command injection
- **Concurrent Execution**: `check_all_applications()` runs checks via `ThreadPoolExecutor` (default 8 workers, `--workers` to override). Each app's output (including nested prints from checker modules, e.g. zigbee2mqtt's MQTT wait) is buffered per-thread and flushed as one atomic write, so concurrent checks can't interleave mid-line. Default output is one condensed summary line per app (`icon name (instance): current -> latest (status)`; apps that track an ESPHome library version show it inline as `2026.6.5 (lib 1.5.8)` via `format_version()`, so a library-only update doesn't look like identical versions flagged as outdated — the same formatting applies to `--list`/`--updates`/`--summary` and the TUI table); pass `--verbose` (CLI) or `verbose=True` to get the full multi-line detail block per app that `--app` single-application checks always show. The shared SQLite connection's write path (`update_row_data`) is guarded by `VersionManager._db_lock` (a `threading.Lock()`) since `sqlite3.Connection` isn't safe for concurrent `execute`/`commit` calls from multiple threads even with `check_same_thread=False`
...
| `notes` | `Notes` | Free-text notes field. The OPNsense check sets it to "Firmware update available" and clears it again once resolved (only that exact string is auto-cleared — other free text is left alone) |

### `transactions` table
One row per upgrade actually triggered (mirrors every `Last_Upgraded` write in `upgrade_rows()`), giving full history instead of a single overwritten timestamp:
...
- **Proxmox**: `config.PROXMOX_API_TOKEN` (required; format `user@realm!tokenid=uuid`); `config.PROXMOX_APT_VERSIONS_URL`/`config.PROXMOX_APT_VERSIONS_NODE` (env-overridable, default `https://pve11.goepp.net:8006`/`pve11`) pick the cluster node whose apt data is used for latest-version checks
- **Home Assistant**: `config.HA_TOKENS` dictionary by instance
- **OPNsense**: `config.OPNSENSE_API_KEY`, `config.OPNSENSE_API_SECRET`
- **AWX**: `config.AWX_API_TOKENS` dict by instance (env: `AWX_API_TOKEN_PROD`, etc.); `config.AWX_BASE_URL` (env: `AWX_BASE_URL`, default `https://awx-prod.goepp.net`)
...
- **AWX base URL**: `config.AWX_BASE_URL` (env: `AWX_BASE_URL`, default `https://awx-prod.goepp.net`). All launches go through one `_launch_awx_job()` helper in `src/checkers/upgrade.py` (the per-method `trigger_awx_*` functions are thin wrappers over it). Job template IDs: k3s Application Update 32, ESPHome 31, server apt upgrade 47, LLM component upgrade 48, UniFi OS Server upgrade (`ui_network_upgrade`) 42; vault upgrades go through **workflow** job template 50 (`/api/v2/workflow_job_templates/`, a different endpoint than the rest)
...
- **`ansible-helm`** upgrade method: behavior depends on `version_pin`. For `latest`: triggers AWX directly, no manifest update. For `pinned`: first updates the file at `helm_values_file` (git add/commit/push, same shape as `ansible-manifest`), then triggers AWX — **except** for `app_name == "vault"`, which instead triggers the dedicated AWX vault-upgrade **workflow** (template 50) via `trigger_vault_upgrade_workflow()`. Vault's `k8s` and `prod` instances share one Helm values file/release, so vault rows are **deferred**: each instance's values update is committed/pushed during the loop, then the workflow fires exactly once at the end of the run (`upgrade_rows()` collects them in `vault_pending`) — this holds no matter how the rows were selected (CLI or TUI multi-select) and guarantees the workflow only runs after every instance's version bump is pushed. All covered rows get `Last_Upgraded` + a transaction; rows beyond the first carry `detail: "covered by shared vault upgrade workflow"`
...
- **`ansible-esphome`** upgrade method: triggers AWX job template 31 (does not wait for job completion due to long compile times). Target selection is run-scoped: if the run contains **every** enabled instance of the app, one job fires with the base `target_pattern` (covering all devices; remaining rows are logged as covered); otherwise each row in the run fires its own targeted `{target}-{instance}` job. Base targets come from `esphome_target_map` (e.g. `konnected` → `garage-door-opener`)
...
- **latest_version**: `"No updates"`, or `"N packages"` / `"N packages + kernel"` (e.g. `"14 packages + kernel"`) — never the literal string "Update available". A failed check (SSH error, nonzero exit) returns `None` so the row shows 📌 Current Version instead of a false ✅ — `"No updates"` always means the check genuinely ran clean
...
## Terminal UI
`check_versions.py --tui` (or `check_versions.py` with no arguments at all — `main()` in `check_versions.py` sets `args.tui = True` when `len(sys.argv) == 1`) launches a full-screen Textual app (`src/tui/app.py`) that wraps `VersionManager` — it calls the exact same `check_all_applications()` / `upgrade_rows()` methods the CLI path uses, no parallel logic. It starts in an "Updates" view (only apps with `status: Update Available`), which is a read filter over `vm.notes`, not a separate data source.
...
- **`u`**: upgrades the selected rows via a **single** `vm.upgrade_rows(idxs)` call (no `force`, so existing skip logic for "already up to date" / kernel-only apt updates still applies); one call for the whole selection so run-scoped dedup works — selecting both vault rows fires the shared workflow once (after both values updates), and selecting every instance of an ESPHome app fires one base-pattern job. Requires confirmation via a modal dialog before running (upgrades can commit/push to the k3s-config repo and trigger AWX jobs, so confirmation is deliberate). Disabled rows in the selection are skipped with a message. After all upgrades are triggered, each affected row is automatically rechecked — since AWX upgrades run asynchronously, this recheck may still show the pre-upgrade version if the job hasn't rolled out yet; use `C` later to check again
- **`U`** (Shift+U): same flow as `u`, but calls `vm.upgrade_rows(..., force=True)`, bypassing the "already up to date" / kernel-only-apt skip checks — mirrors CLI `--upgrade --force`. Useful when a prior upgrade attempt already bumped the version but failed partway, so a plain `u` would be skipped as a no-op. Bound as literal `"U"`, not `"shift+u"`, for the same terminal-keyboard-protocol reason as `C` (see above). Same confirmation modal as `u`, with "Force upgrade" in the prompt text
...
6. Import the checker function in `version_manager.py` and add a one-line entry to the `CURRENT_CHECKERS` registry (use the `_api_checker`/`_kubectl_checker` adapters for standard signatures)
...
- **Import pattern**: Import checker functions in `version_manager.py` and register them in `CURRENT_CHECKERS`
- **Unified approach**: `linux_kernel.py` handles all Linux distributions
- **Shared ESPHome device access**: `src/checkers/esphome_device.py` owns the aioesphomeapi connect/read logic; `ble_proxy.py`, `konnected.py`, and `airgradient.py` are thin wrappers over it
...
- **API Efficiency**: GitHub and Docker Hub API calls are cached using @lru_cache to dedupe lookups for multi-instance apps; caches are cleared at the start of each check-all run
